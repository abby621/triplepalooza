...
def main(margin,batch_size,output_size,learning_rate,whichGPU,is_finetuning,pretrained_net):
    ...
    ckpt_dir = './output/traffickcam/ckpts/finetuning_netvlad'
    ...
    for var in tf.trainable_variables():
        excluded = False
        if is_finetuning.lower() == 'true' and ('rgb' in var.op.name.lower() or 'wpca' in var.op.name.lower() or 'l2_normalize' in var.op.name.lower()):
            excluded = True
        if not excluded:
